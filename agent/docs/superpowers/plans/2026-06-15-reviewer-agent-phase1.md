...
- **Next task:** Task 16 (`common/` extraction) — **RECOMMEND deferring to a focused fresh session.** It's a broad cross-package import rewrite (stock_agent + backtest + review_agent) with zero new behavior; safest done with fresh context to catch every import site without regressing the trading system. The current `review_agent → stock_agent` import dependency is acceptable interim (the capability boundary is enforced by the tool allowlist, not packaging). Then **Phase 2** = skills A2, C1, B1, B2, A4.
