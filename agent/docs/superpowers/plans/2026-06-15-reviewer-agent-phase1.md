...
- **Next task:** Task 16 (`common/` extraction) — attempting now, BOUNDED scope (move `supabase_client.py` + `db.py` to `common/`, repoint imports) with exhaustive verification + bail-out. If too broad/risky, keep documented interim (`review_agent → stock_agent`) — boundary is enforced by the tool allowlist regardless.
